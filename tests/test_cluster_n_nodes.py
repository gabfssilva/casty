...
class TestClusterFormation:
    ...
    async def test_n_node_cluster_forms(self, num_nodes: int):
        """N nodes should form a cluster and see each other."""
        async with DevelopmentCluster(num_nodes) as systems:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_cluster_membership_visible(self, num_nodes: int):
        """Each node should see all other members via SWIM."""
        async with DevelopmentCluster(num_nodes) as systems:
            await asyncio.sleep(0.5 + 0.2 * num_nodes)

            # Each node should have a cluster connection
...
class TestShardedActorsNNodes:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_sharded_entity_distribution(self, num_nodes: int):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_sharded_entity_ask_from_any_node(self, num_nodes: int):
        ...
        async with DevelopmentCluster(num_nodes) as systems:
            ...
            for entity_id in test_entities:
                ...
                assert value == 10, f"Entity {entity_id} has wrong value: {value}"

    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_concurrent_entity_updates(self, num_nodes: int):
        ...
        async with DevelopmentCluster(num_nodes) as systems:
            ...
            tasks = [update_entity(f"entity-{i}") for i in range(num_entities)]
...
class TestClusterThroughput:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_entity_ask_throughput(self, num_nodes: int):
...
class TestClusterFaultTolerance:
    ...
    async def test_cluster_survives_node_loss(self, num_nodes: int):
        ...
        async with DevelopmentCluster(num_nodes) as systems:
            ...
            # Shutdown the last node manually
            await systems[-1].shutdown()
...
class TestClusterConsistency:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_entity_state_consistency(self, num_nodes: int):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_nodes", [2, 3, 5])
    async def test_sequential_updates_ordered(self, num_nodes: int):
