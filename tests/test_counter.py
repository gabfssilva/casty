...
from casty.distributed.counter import counter_entity
...
async def test_counter_increment_and_get() -> None:
    ...
    async with ActorSystem(name="counter-test") as system:
        ...
        counter = Counter(
            gateway=system, region_ref=region_ref, name="my-counter", timeout=5.0
        )
...
async def test_counter_multiple_counters() -> None:
    ...
    async with ActorSystem(name="multi-counter-test") as system:
        ...
        counter_a = Counter(
            gateway=system, region_ref=region_ref, name="counter-a", timeout=5.0
        )
        counter_b = Counter(
            gateway=system, region_ref=region_ref, name="counter-b", timeout=5.0
        )
