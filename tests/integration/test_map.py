...
async def test_map_full_api_across_nodes_and_client() -> None:
    ...
    try:
        stock: casty.Map[str, int] = systems[0].map("stock")
        for i in range(40):
        ...
        # same map seen from another system
        stock_b: casty.Map[str, int] = systems[1].map("stock")
...
async def test_map_message_values_and_distribution() -> None:
    systems = await start_nodes(3)
    try:
        catalog: casty.Map[str, Sku] = systems[0].map("catalog", shards=8)
        for i in range(16):
        ...
        hosting = [
            sum(1 for (wire, _key) in system._host._activations if "MapShard" in wire)
...
async def test_map_survives_owner_death() -> None:
    systems = await start_nodes(3)
    try:
        stock: casty.Map[str, int] = systems[0].map("durable-map", shards=4)
        for i in range(12):
...
async def test_map_put_fenced_in_minority() -> None:
    ...
    try:
        stock: casty.Map[str, int] = systems[0].map("fenced-map", shards=1)
        await stock.put("k", 1)
        info = systems[0].map("fenced-map", shards=1)._info
