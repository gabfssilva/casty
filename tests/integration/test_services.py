...
def _hosts(nodes: list[casty.Node]) -> list[casty.Node]:
    return [n for n in nodes if n._host.is_active(WIRE, "@")]


async def test_node_service_dispatches_locally() -> None:
    nodes = await start_nodes(3)
    try:
        assert await nodes[1].service(Fetcher).slow(0.0, 21) == 42
        assert _hosts(nodes) == [nodes[1]]  # zero hops: the caller hosts the coordinator
    finally:
...
async def test_service_reaches_a_replicated_actor() -> None:
    ...
    try:
        ...
        assert await nodes[2].actor(Counter, "sku:1").read() == 10  # same key serializes
    finally:
        await stop_all(nodes)
...
async def test_client_load_balances_across_members() -> None:
    ...
    try:
        ...
        assert len(_hosts(nodes)) == 3  # round-robin: every member served some calls
    finally:
        await client.close()
        await stop_all(nodes)
...
async def test_in_flight_calls_fail_when_the_host_dies() -> None:
    ...
    try:
        ...
        await client.close()
        await stop_all([n for n in nodes if n is not victim])
