...
async def test_semaphore_full_api_across_nodes_and_client() -> None:
    ...
    try:
        sem: casty.Semaphore = nodes[0].semaphore("gate", capacity=2)
        ...
        # bound reached: same semaphore seen from another node also refuses
        sem_b: casty.Semaphore = nodes[1].semaphore("gate", capacity=2)
...
async def test_semaphore_capacity_bound_under_contention() -> None:
    nodes = await start_nodes(3)
    try:
        sem: casty.Semaphore = nodes[0].semaphore("bounded", capacity=3)
...
async def test_semaphore_lease_expiry_reclaims_permit() -> None:
    nodes = await start_nodes(3)
    try:
        sem: casty.Semaphore = nodes[0].semaphore("leased", capacity=1)
...
async def test_semaphore_acquire_blocks_until_release() -> None:
    nodes = await start_nodes(3)
    try:
        sem: casty.Semaphore = nodes[0].semaphore("blocking", capacity=1)
...
async def test_semaphore_survives_owner_death() -> None:
    nodes = await start_nodes(3)
    try:
        info = nodes[0].semaphore("durable", capacity=2)._info
        owner = owner_of(nodes, info.wire_name, "durable:0")
        survivor = next(n for n in nodes if n is not owner)
        ...
        await wait_view([n for n in nodes if n is not owner], 2)
...
async def test_semaphore_acquire_fenced_in_minority() -> None:
    ...
    try:
        info = nodes[0].semaphore("fenced", capacity=1)._info
        owner = owner_of(nodes, info.wire_name, "fenced:0")
