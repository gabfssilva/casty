...
def _stream_out(
    ...
    *,
    ...
    return system._stream_out(_info(), key, _info().methods[method], kwargs or {}, in_iter, [])
...
async def _stream_in(
    ...
    *,
    ...
    return await system._stream_in(_info(), key, _info().methods[method], kwargs or {}, in_iter, [])
...
def _remote(systems: list[casty.Node], key: str) -> casty.Node:
    owner = owner_of(systems, WIRE, key)
    return next(n for n in systems if n is not owner)
...
async def test_server_streaming_cross_node() -> None:
    ...
    try:
        ...
        assert owner_of(systems, WIRE, key)._host.is_active(WIRE, key)
    finally:
...
async def test_local_and_remote_both_work() -> None:
    ...
    try:
        key = "both"
        owner = owner_of(systems, WIRE, key)
        remote = next(n for n in systems if n is not owner)
...
async def test_owner_death_mid_stream_raises() -> None:
    ...
    try:
        ...
        await stop_all([n for n in systems if n is not owner])
