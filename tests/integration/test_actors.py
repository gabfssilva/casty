...
async def test_same_key_hits_same_activation_across_nodes() -> None:
    ...
    try:
        for i, node in enumerate(nodes):
            counter = node.actor(Counter, "shared")
            assert await counter.add(1) == i + 1
        assert await nodes[0].actor(Counter, "shared").read() == 3
    finally:
...
async def test_keys_distribute_across_nodes() -> None:
    ...
    try:
        for i in range(60):
            await nodes[0].actor(Counter, f"key-{i}").add(1)
        hosting = [n._host.activation_count for n in nodes]
...
async def test_owner_death_reactivates_elsewhere() -> None:
    ...
    try:
        key = "reborn"
        await nodes[0].actor(Counter, key).add(10)
        wire = "itest.Counter"
        owner = next(n for n in nodes if n._host.is_active(wire, key))
...
async def test_reentrancy_cycle_raises_and_plain_chain_works() -> None:
    ...
    try:
        ...
        result = await nodes[0].actor(ChainActor, "a").ping(["b", "c"])
        assert result == "c"
        ...
        with pytest.raises(ActorFailedError, match="ReentrancyError"):
            await nodes[0].actor(ChainActor, "a").ping(["b", "a"])
        # a -> a: self-cycle, same shape
        with pytest.raises(ActorFailedError, match="ReentrancyError"):
            await nodes[0].actor(ChainActor, "a").ping(["a"])
    finally:
        await stop_all(nodes)
...
async def test_idle_deactivation_and_explicit_deactivate() -> None:
    ...
    try:
        fx.events.clear()
        actor = nodes[0].actor(Lifecycle, "idle-key")
        ...
        fx.events.clear()
        other = nodes[0].actor(Lifecycle, "retire-key")
        await other.touch()
        await other.retire()
        await asyncio.sleep(0.1)
...
async def test_supervisor_directives() -> None:
    ...
    nodes = await start_nodes(1, supervisor=supervisor)
    try:
        node = nodes[0]
        ...
        with pytest.raises(ActorFailedError):
            await stop.boom()
        assert not node._host.is_active("itest.Flaky", "stop")
...
async def test_graceful_stop_drains_and_leaves_cleanly() -> None:
    ...
    try:
        ...
        for node in survivors:
            node.membership.subscribe(record)
...
async def test_lite_member_routes_and_stays_out_of_the_ring() -> None:
    ...
    try:
        client = await casty.connect([nodes[0].member.addr], config=fx.FAST_CONFIG)
        counter = client.actor(Counter, "via-client")
        assert await counter.add(5) == 5
        assert await nodes[0].actor(Counter, "via-client").read() == 5
        for node in nodes:
            ring = node._ring
