...
async def test_counter_full_api_across_nodes_and_client() -> None:
    ...
    try:
        hits: casty.Counter = nodes[0].counter("hits")
        for i in range(10):
            await nodes[i % 3].counter("hits").add(i)
        assert await hits.get() == sum(range(10))

        # same counter seen from another node
        hits_b: casty.Counter = nodes[1].counter("hits")
        await hits_b.add(5)
...
async def test_counter_concurrent_adds_sum_correctly() -> None:
    nodes = await start_nodes(3)
    try:
        total: casty.Counter = nodes[0].counter("concurrent", stripes=8)
        n = 200
        delta = 3
...
async def test_counter_survives_owner_death() -> None:
    nodes = await start_nodes(3)
    try:
        total: casty.Counter = nodes[0].counter("durable-counter", stripes=4)
        for i in range(12):
            await total.add(i)
...
async def test_counter_add_fenced_in_minority() -> None:
    ...
    try:
        total: casty.Counter = nodes[0].counter("fenced-counter", stripes=1)
        await total.add(1)
        info = nodes[0].counter("fenced-counter", stripes=1)._info
        ring = nodes[0]._ring
...
async def test_counter_stripes_distribute_over_nodes() -> None:
    nodes = await start_nodes(3)
    try:
        spread: casty.Counter = nodes[0].counter("spread", stripes=16)
        for _i in range(16):
            await spread.add(1)
        hosting = [
            sum(1 for (wire, _key) in node._host._activations if "CounterShard" in wire)
            for node in nodes
        ]
