...
async def test_set_full_api_across_nodes_and_client() -> None:
    ...
    try:
        colors: casty.Set[str] = systems[0].set("colors")
        for i in range(40):
        ...
        # same set seen from another system
        colors_b: casty.Set[str] = systems[1].set("colors")
...
async def test_set_message_values_and_distribution() -> None:
    systems = await start_nodes(3)
    try:
        tags: casty.Set[Tag] = systems[0].set("tags", shards=8)
        for i in range(16):
        ...
        hosting = [
            sum(1 for (wire, _key) in system._host._activations if "SetShard" in wire)
...
async def test_set_survives_owner_death() -> None:
    systems = await start_nodes(3)
    try:
        colors: casty.Set[str] = systems[0].set("durable-set", shards=4)
        for i in range(12):
...
async def test_set_add_fenced_in_minority() -> None:
    ...
    try:
        colors: casty.Set[str] = systems[0].set("fenced-set", shards=1)
        await colors.add("c")
        info = systems[0].set("fenced-set", shards=1)._info
...
async def test_set_algebra() -> None:
    systems = await start_nodes(3)
    try:
        a: casty.Set[int] = systems[0].set("set-a")
        b: casty.Set[int] = systems[0].set("set-b")
        for i in range(10):
            await a.add(i)
        for i in range(5, 15):
            await b.add(i)
