...
@dataclass(frozen=True)
class Starts(Askable[int]):
    pass


@dataclass(frozen=True)
class Explode(Askable[int]):
    pass
...
@dataclass(frozen=True)
class Count(Askable[tuple[str, int]]):
    pass
...
@dataclass(frozen=True)
class Echo(Askable[Sample]):
    sample: Sample


@dataclass(frozen=True)
class Recall(Askable[Sample | None]):
    pass


@dataclass(frozen=True)
class Ignore(Askable[int]):
    pass


@dataclass(frozen=True)
class Nap(Askable[int]):
    pass
...
@dataclass(frozen=True)
class Shout(Askable[str]):
    text: str


@dataclass(frozen=True)
class Counted[T](Askable[T]):
    """An askable whose answer is set by a subclass, one generic class further down."""


@dataclass(frozen=True)
class Tallied(Counted[int]):
    pass


@dataclass(frozen=True)
class Descend(Askable[tuple[str, ...]]):
    depth: int
...
@dataclass(frozen=True)
class Unplan(Askable[tuple[datetime | None, ...]]):
    pass


@dataclass(frozen=True)
class Plans(Askable[tuple[tuple[str, timedelta | None, datetime | None], ...]]):
    pass


@dataclass(frozen=True)
class Replan(Askable[tuple[datetime | None, int]]):
    pass
...
@dataclass(frozen=True)
class Misplan(Askable[tuple[str, ...]]):
    pass
...
async def planner(ctx: Context[tuple[str, ...], Planning]) -> None:
    async for msg in ctx.inbox:
        match msg:
            ...
            case Plans(reply_to=reply_to):
                reply_to.tell(_plans(ctx))
            case Replan(reply_to=reply_to):
            ...
            case Misplan(reply_to=reply_to):
                refused: list[str] = []
...
def describe_actor_system() -> None:
            ...
            async def fragile(ctx: Context[Account, Deposit | Explode | Inspect]) -> None:
                ...
                async for msg in ctx.inbox:
                    ...
                    match msg:
                        ...
                        case Explode():
                            raise RuntimeError("boom")
                        case Inspect(reply_to=reply_to):
        ...
        async def it_runs_once_its_ref_is_obtained_and_resumes_from_the_saved_offset() -> None:
            ...
            async def consumer(ctx: Context[Offset, GetOffset]) -> None:
                async for item in ctx.merge(feed(ctx.state.value.value)):
                    match item:
                        case GetOffset(reply_to=reply_to):
    ...
    def when_messages_carry_every_supported_type() -> None:
        async def it_delivers_them_unchanged() -> None:
            ...
            async def echo(ctx: Context[Stash, Echo | Recall]) -> None:
                async for msg in ctx.inbox:
                    match msg:
                        case Echo(sample, reply_to=reply_to):
                            await ctx.state.set(Stash(sample))
                            reply_to.tell(sample)
                        case Recall(reply_to=reply_to):
    ...
    def when_a_message_is_askable() -> None:
        async def it_gives_each_ask_a_reply_to_of_its_own_and_leaves_the_message_passed_as_it_was() -> None:
            @actor(initial=Account())
            async def shouting(ctx: Context[Account, Shout]) -> None:
                async for msg in ctx.inbox:
                    msg.reply_to.tell(msg.text.upper())

            async with ActorSystem() as system:
                ref = system.ref(shouting, "k")
                hi = Shout("hi")
                assert await asyncio.gather(ref.ask(hi), ref.ask(hi)) == ["HI", "HI"]
                assert repr(hi.reply_to) == "Ref(nobody)"

        async def it_drops_the_answer_to_one_told_without_a_reply_to() -> None:
            @actor(initial=tuple[str, ...]())
            async def shouting(ctx: Context[tuple[str, ...], Shout | Heard]) -> None:
                async for msg in ctx.inbox:
                    match msg:
                        case Shout(text):
                            msg.reply_to.tell(text.upper())
                            await ctx.state.update(lambda heard: (*heard, text))
                        case Heard():
                            msg.reply_to.tell(ctx.state.value)
            ...
            async with ActorSystem() as system:
                ref = system.ref(shouting, "k")
                ref.tell(Shout("hi"))
                assert repr(Shout("hi").reply_to) == "Ref(nobody)"

                async def shouted() -> None:
                    assert await ref.ask(Heard()) == ("hi",)

                await eventually(shouted)

        async def it_answers_the_ref_it_was_built_with_when_it_is_told() -> None:
            @actor(initial=Account())
            async def shouting(ctx: Context[Account, Shout]) -> None:
                async for msg in ctx.inbox:
                    msg.reply_to.tell(msg.text.upper())
            ...
            @actor(initial=tuple[str, ...]())
            async def listening(ctx: Context[tuple[str, ...], str | Heard]) -> None:
                async for msg in ctx.inbox:
                    match msg:
                        case str() as text:
                            await ctx.state.update(lambda heard: (*heard, text))
            ...
            async with ActorSystem() as system:
                listener = system.ref(listening, "k")
                system.ref(shouting, "k").tell(Shout("hi", reply_to=listener))

                async def heard() -> None:
                    assert await listener.ask(Heard()) == ("HI",)

                await eventually(heard)

        async def it_lets_the_key_it_is_handed_on_to_answer_it() -> None:
            @actor(initial=Account())
            async def shouting(ctx: Context[Account, Shout]) -> None:
                async for msg in ctx.inbox:
                    msg.reply_to.tell(f"{ctx.key}: {msg.text}")

            @actor(initial=Account())
            async def forwarding(ctx: Context[Account, Shout]) -> None:
                async for msg in ctx.inbox:
                    ctx.system.ref(shouting, "other").tell(msg)

            async with ActorSystem() as system:
                assert await system.ref(forwarding, "k").ask(Shout("hi")) == "other: hi"

        async def it_reads_the_answer_as_the_type_a_generic_base_is_given_further_down() -> None:
            @actor(initial=Account())
            async def tallying(ctx: Context[Account, Tallied]) -> None:
                async for msg in ctx.inbox:
                    msg.reply_to.tell(7)

            async with ActorSystem() as system:
                assert await system.ref(tallying, "k").ask(Tallied()) == 7
    ...
    def when_the_body_hands_work_to_itself() -> None:
        ...
        async def it_asks_and_takes_what_mapper_or_failed_make_of_the_answer_as_a_message() -> None:
            released = asyncio.Event()
            ...
            async def shouting(ctx: Context[Account, Shout]) -> None:
                async for msg in ctx.inbox:
                    await released.wait()
                    if msg.text == "boom":
                        raise ValueError("no shout")
                    msg.reply_to.tell(msg.text.upper())

            @actor(initial=tuple[str, ...]())
            async def asking(ctx: Context[tuple[str, ...], Handing]) -> None:
                async for msg in ctx.inbox:
                    match msg:
                        case Begin():
                            shouts = ctx.system.ref(shouting, ctx.key)
                            ctx.ask(shouts, Shout("hi"), Note)
                            ctx.ask(shouts, Shout("boom"), mapper=Note, failed=lambda error: Note(type(error).__name__))
                        case _:
                            await _noted(ctx, msg)

            async with ActorSystem() as system:
                ref = system.ref(asking, "k")
                ref.tell(Begin())
                # Answered while `shouting` still holds both answers back.
                assert await ref.ask(Heard()) == ()
                released.set()

                async def noted() -> None:
                    assert await ref.ask(Heard()) == ("ActorFailed", "HI")

                await eventually(noted)

        async def it_reports_a_dropped_message_naming_the_ask_when_it_raises_and_nothing_maps_it() -> None:
            events: list[Event] = []
            ...
            async def asking(ctx: Context[tuple[str, ...], Handing]) -> None:
                async for msg in ctx.inbox:
                    match msg:
                        case Begin():
                            ctx.ask(ctx.system.ref(shouting, ctx.key), Shout("hi"), Note)
                        case _:
                            await _noted(ctx, msg)

            async with ActorSystem(observer=events.append) as system:
                system.ref(asking, "k").tell(Begin())

                async def reported() -> None:
                    dropped = [event for event in events if isinstance(event, MessageDropped)]
                    assert [(event.actor, event.key) for event in dropped] == [(asking.name, "k")]
                    assert dropped[0].reason.startswith(f"the ask to Ref({shouting.name}/k) raised ActorFailed")
